...
const GlobalNav = () => {
    ...
    const getLinkClass = (path: string) => {
        ...
        return `p-2 rounded tooltip transition-colors ${
            isActive ? 'bg-primary text-white shadow-lg shadow-primary/40' : 'text-white/60 hover:text-white hover:bg-white/10'
        }`;
    };

    return (
        <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-[#1a202c]/90 backdrop-blur border border-border-dark px-2 py-2 rounded-xl shadow-2xl flex gap-1 animate-fade-in-up">
